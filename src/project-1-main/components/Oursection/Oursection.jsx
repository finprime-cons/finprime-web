  ...
  return (
    <div
      ...
      <div className='absolute left-0 right-0 flex justify-center p-4 bottom-16'>
        <h4 className='font-bold text-white text-md sm:text-lg font-raleway lg:text-xl'>{title}</h4>
      </div>
    </div>
  );
};
...
const OurSection = () => {
  ...
  return (
    <div
      id='ceo'
      ref={fadeElementRef}
      className={`mt-5 lg:mt-16 sm:mx-6 md:ml-8 md:mr-6 lg:mr-8 lg:ml-10 xl:mr-10 xl:ml-12 ml-6 mr-4  ${hasFadedIn ? 'animate-fadeinbottom' : ''}`}>
      <div className='px-2 pt-8 pb-4 bg-pink-50 lg:pb-8 sm:mx-0 sm:px-8 md:px-6 lg:px-10'>
        <div className='flex flex-col justify-between mx-2 mb-1 text-center sm:flex-row'>
          <h4 className='mb-0 text-xl font-semibold text-center body sm:text-2xl font-khula sm:text-left sm:mb-0'>Relax. We've got you.</h4>
          <h5 className='mt-2 text-center text-md body sm:text-2xl font-khula sm:text-right'>The best results</h5>
        </div>
        <div className='flex flex-wrap justify-center gap-0'>
          {cardData.map((card, index) => (
            <Link to={card.link} className='w-full p-2 cursor-pointer sm:w-1/2 md:w-1/3' key={index}>
