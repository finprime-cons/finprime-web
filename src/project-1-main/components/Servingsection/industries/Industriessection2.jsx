...
const Industriessection2 = () => {
  ...
  return (
    <div className="bg-white mt-[300px] sm:mt-[400px] mb-[150px]   xl:pl-12 xl:pr-10 lg:pl-10 lg:pr-8 md:pl-8 md:pr-6 pr-4 pl-6">
      <div id="service-two">
        <div className="flex pb-10 text-3xl font-semibold text-black sm:text-5xl font-khula">
          <div className="text-6xl sm:text-8xl font-extralight ">|</div>{" "}
          <div className="pb-5">{subIndustry.headtitle}</div>
        </div>
      </div>
      <div className="flex flex-col ">
        <div className="w-full  font-raleway sm:px-2 text-md md:text-[16px] tracking-[1px] leading-7  text-black">
          <p className="mb-12 ">{subIndustry.content}</p>
          <Link
        ...
        <div className="flex flex-wrap mt-28 ">
          {industry.subindustries.map((sub) => (
            <Link
              to={`/${encodeURIComponent(industry.title)}/${encodeURIComponent(
                sub.title
              )}`}
              key={sub.id}
            ...
            >
              <span id="subimdustry" className="text-center ">
                {sub.headtitle}
              </span>
            </Link>
          ))}
