      ...
      <div className="grid grid-cols-1 gap-8 md:grid-cols-3 wrapper">
        {visibleColumns.map((subIndustry) => (
          ...
          >
            ...
            />
            <div className="info">
              <h4 className="text-xl font-bold mb-5 sm:text-[28px] text-white">
                {subIndustry.headtitle}
              </h4>
              <p className="text-white font-raleway tracking-[1px] text-[16px]">
                {subIndustry.content.split(" ").slice(0, 10).join(" ")}
                {subIndustry.content.split(" ").length > 10 ? "..." : ""}
              </p>
            </div>
          </Link>
        ))}
