...
const SendEmail = async (req, res) => {
  try {
    ...
    const mobileRegex = /^\d{10}$/; // Adjust based on country format
    ...
    const query = `
      ...
      VALUES (?, ?, ?, ?)
    `;
    await queryAsync(query, [name, company, email, mobile]);
...
const GetMails = async (req, res) => {
  try {
    ...
    const query = `SELECT id, name, company, email, mobile, created_at FROM inquiries ORDER BY submitted_at DESC`;
